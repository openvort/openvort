...
import { createEmbeddingProvider, deleteEmbeddingProvider, getEmbeddingProviders, updateEmbeddingProvider } from "@/api";
...
const editingId = ref("");
const list = ref<EmbeddingProviderItem[]>([]);
...
async function loadData() {
    loading.value = true;
    try {
        const res: any = await getEmbeddingProviders();
        list.value = Array.isArray(res?.providers) ? res.providers : [];
    } catch {
        message.error("加载向量服务商失败");
    } finally {
        loading.value = false;
    }
}
...
async function handleSave() {
    ...
    try {
        if (editing.value) {
            ...
            await updateEmbeddingProvider(editingId.value, payload);
...
async function handleToggleEnabled(row: EmbeddingProviderItem) {
    try {
        await updateEmbeddingProvider(row.id, { is_enabled: !row.is_enabled });
...
async function handleSetDefault(row: EmbeddingProviderItem) {
    if (row.is_default) return;
    try {
        await updateEmbeddingProvider(row.id, { is_default: true });
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            <p class="text-sm text-gray-500">
                管理向量嵌入（Embedding）服务商配置。知识库文档处理需要向量服务支持。
            </p>
            <VortButton variant="primary" @click="handleAdd">
                <Plus :size="14" class="mr-1" /> 新增向量服务
            </VortButton>
        </div>
        ...
        <VortDialog :open="dialogOpen" :title="editing ? '编辑向量服务' : '新增向量服务'" @update:open="dialogOpen = $event">
            <VortForm label-width="110px" class="mt-2">
                <VortFormItem label="名称" required>
                    <VortInput v-model="form.name" placeholder="如 阿里云-Embedding" />
                </VortFormItem>
