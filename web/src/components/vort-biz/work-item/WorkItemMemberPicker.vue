...
const fallbackAvatarLabel = (name: string): string =>
  name.slice(0, 1).toUpperCase();

const getAvatarBg = (name: string) =>
  props.getAvatarBg?.(name) || fallbackAvatarBg(name);
const getAvatarLabel = (name: string) =>
  props.getAvatarLabel?.(name) || fallbackAvatarLabel(name);
    ...
    .map((group) => ({
      ...
      members: group.members
        .map((member, index) => ({
          ...
          index,
          score:
            props.filterScore?.(member, kw) ??
            getDefaultFilterScore(member, kw),
        }))
...
const isCollaborator = (member: string) =>
  currentCollaborators.value.includes(member);

const emitChange = (
  nextOwner = currentOwner.value,
  nextCollaborators = currentCollaborators.value,
) => {
  emit("change", {
    owner: nextOwner,
...
const setOwner = (member: string) => {
  ...
  const nextCollaborators = currentCollaborators.value.filter(
    (item) => item !== member,
  );
  ...
  >
    <template #trigger="{ open: triggerOpen }">
      ...
      >
        <button
          type="button"
          class="member-picker-default-trigger"
          :disabled="disabled"
        >
    ...
    <div class="member-picker-content">
      ...
      >
        ...
        <div
          ...
          :class="{
            'is-active':
              mode === 'owner' ? isOwner(member) : isCollaborator(member),
            'is-owner-row': mode === 'assignee' && isOwner(member),
            'is-collaborator-row':
              mode === 'assignee' && !isOwner(member) && isCollaborator(member),
          }"
        >
          ...
          <template v-else>
            <div
              class="member-picker-row-left"
              @click.stop="
                mode === 'owner' ? setOwner(member) : toggleCollaborator(member)
              "
            >
              <vort-checkbox
                :checked="
