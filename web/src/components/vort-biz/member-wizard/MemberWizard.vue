...
watch(currentStep, async (step) => {
    if (step === 3 && formData.value.memberType === "virtual") {
        await loadRoleSkills();
    }
});
...
function nextStep() {
    ...
    if (currentStep.value === 2 && formData.value.memberType === "virtual") {
        if (formData.value.roles.length === 0) {
...
// 步骤标题
const stepTitles = ["", "选择类型", "选择角色", "确认技能", "填写信息"];
</script>

<template>
    <VortDialog
        :open="open"
        :title="`添加成员 - ${stepTitles[currentStep]}`"
        width="720px"
        :footer="false"
    ...
    >
        ...
        <!-- Step 2: 选择角色（仅虚拟员工） -->
        <div v-if="currentStep === 2 && formData.memberType === 'virtual'" class="space-y-4">
            <div class="text-center mb-6">
        ...
        <!-- Step 2: 真实员工直接跳过到 Step 4 -->
        <div v-if="currentStep === 2 && formData.memberType === 'real'" class="text-center py-8">
            <div class="text-gray-500">真实员工无需配置角色和技能</div>
            <div class="text-sm text-gray-400 mt-2">点击"下一步"继续填写基本信息</div>
        </div>

        <!-- Step 3: 确认技能（仅虚拟员工） -->
        <div v-if="currentStep === 3 && formData.memberType === 'virtual'" class="space-y-4">
            <div class="text-center mb-6">
                ...
                <div v-if="availableSkills.length" class="space-y-2 max-h-80 overflow-y-auto">
                    ...
                    >
                        <VortCheckbox
                            ...
                            @change="() => {
                                const idx = formData.value.skills.findIndex(s => s.id === skill.id);
                                if (skill.checked && idx === -1) {
                                    formData.value.skills.push(skill);
                                } else if (!skill.checked && idx > -1) {
