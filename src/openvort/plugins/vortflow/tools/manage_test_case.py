...
class ManageTestCaseTool(BaseTool):
    ...
    def input_schema(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "case_type": {
                    "type": "string",
                    "enum": ["functional", "performance", "api", "ui", "security"],
                    "description": "用例类型",
                },
                "priority": {
                    "type": "integer",
                    "enum": [0, 1, 2, 3],
                    "description": "优先级 (0=P0, 1=P1, 2=P2, 3=P3)",
                },
                ...
                "entity_type": {
                    "type": "string",
                    "enum": ["story", "task", "bug"],
                    "description": "工作项类型（关联时使用）",
                },
                "entity_id": {"type": "string", "description": "工作项 ID（关联时使用）"},
    ...
    async def execute(self, params: dict) -> str:
        ...
        if action == "create_module":
            ...
            return json.dumps({
                "ok": True, "message": f"模块「{name}」已创建", "module_id": mod.id,
            }, ensure_ascii=False)
            ...
            async with sf() as session:
                ...
                if "parent_id" in params:
                    mod.parent_id = params["parent_id"] or None
                    changes["parent_id"] = params["parent_id"]
                if not changes:
                    return json.dumps({"ok": False, "message": "未提供任何修改字段"})
                await session.commit()
            return json.dumps({
                "ok": True, "message": f"模块「{mod.name}」已更新",
            }, ensure_ascii=False)
            ...
            async with sf() as session:
                rows = (await session.execute(
                    select(FlowTestModule)
                    .where(FlowTestModule.project_id == project_id)
                    .order_by(FlowTestModule.sort_order, FlowTestModule.created_at)
                )).scalars().all()
                ...
                items = [{
                    ...
                    "sort_order": m.sort_order, "case_count": counts.get(m.id, 0),
                } for m in rows]
        ...
        # ---- Case actions ----
            ...
            if params.get("maintainer_name"):
                async with sf() as session:
                    result = await session.execute(
                        select(Member).where(Member.name == params["maintainer_name"])
                    )
            ...
            async with sf() as session:
                tc = FlowTestCase(
                    ...
                    module_id=params.get("module_id") or None,
                    title=title,
                    precondition=params.get("precondition", ""),
                    notes=params.get("notes", ""),
                    case_type=params.get("case_type", "functional"),
            ...
            return json.dumps({
                "ok": True, "message": f"用例「{title}」已创建", "case_id": tc.id,
            }, ensure_ascii=False)
            ...
            async with sf() as session:
                ...
                if params.get("maintainer_name"):
                    ...
                    m = result.scalar_one_or_none()
                    if m:
                        tc.maintainer_id = m.id
                        changes["maintainer"] = params["maintainer_name"]
            ...
            return json.dumps({
                "ok": True,
                "message": f"用例「{tc.title}」已更新，修改字段: {'、'.join(changes.keys())}",
            }, ensure_ascii=False)
            ...
            async with sf() as session:
                stmt = select(FlowTestCase).where(
                    FlowTestCase.project_id == project_id
                ).order_by(FlowTestCase.created_at.desc()).limit(50)

                if params.get("module_id"):
                    stmt = stmt.where(FlowTestCase.module_id == params["module_id"])
                if params.get("case_type"):
                    stmt = stmt.where(FlowTestCase.case_type == params["case_type"])
                if params.get("priority") is not None:
                    stmt = stmt.where(FlowTestCase.priority == params["priority"])
                if params.get("keyword"):
                    stmt = stmt.where(FlowTestCase.title.ilike(f"%{params['keyword']}%"))
                ...
                items = [{
                    ...
                    "priority": tc.priority, "module_id": tc.module_id,
            ...
            return json.dumps({
                ...
                "case": {
                    ...
                    "module_id": tc.module_id, "module_name": module_name,
                    "title": tc.title, "precondition": tc.precondition,
                    "notes": tc.notes, "case_type": tc.case_type,
                    "priority": tc.priority, "maintainer_name": maintainer_name,
                    "review_result": tc.review_result, "steps": steps,
                    "linked_work_item_count": link_count,
        ...
        # ---- Link actions ----
            ...
            async with sf() as session:
                existing = (await session.execute(
                    select(FlowTestCaseWorkItem).where(
                        FlowTestCaseWorkItem.test_case_id == case_id,
                ...
                link = FlowTestCaseWorkItem(
                    test_case_id=case_id, entity_type=entity_type,
            ...
            return json.dumps({
                "ok": True, "message": "关联已创建", "link_id": link.id,
            }, ensure_ascii=False)
            ...
            async with sf() as session:
                ...
                if case_id:
                    stmt = stmt.where(FlowTestCaseWorkItem.test_case_id == case_id)
