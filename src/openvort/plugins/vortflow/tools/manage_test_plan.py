...
创建、修改、删除测试计划，关联/移除用例，记录执行结果，查询计划列表和详情。
"""
...
class ManageTestPlanTool(BaseTool):
    ...
    async def execute(self, params: dict) -> str:
        ...
        if action == "create":
            ...
            return await self._delete(
                params, sf, FlowTestPlan, FlowTestPlanCase,
            ...
            return await self._update_review(
                params, sf, member_id, Member, FlowTestPlanReview, FlowTestPlanReviewHistory,
            )
            ...
            return await self._list(
                params, sf, Member, FlowTestPlan, FlowTestPlanCase,
    ...
    async def _create(self, params, sf, member_id, Member, FlowTestPlan):
        ...
        async with sf() as session:
            if owner_name:
                result = await session.execute(select(Member).where(Member.name == owner_name))
            ...
            plan = FlowTestPlan(
                ...
                owner_id=owner_id or (member_id or None),
                iteration_id=params.get("iteration_id") or None,
                version_id=params.get("version_id") or None,
    ...
    async def _update(self, params, sf, member_id, Member, FlowTestPlan):
        ...
        async with sf() as session:
            ...
            for fk_field in ("iteration_id", "version_id"):
                if fk_field in params and params[fk_field] is not None:
                    setattr(plan, fk_field, params[fk_field] or None)
                    changes[fk_field] = params[fk_field]
        ...
        return json.dumps({
            "ok": True,
            "message": f"测试计划「{plan.title}」已更新，修改字段: {'、'.join(changes.keys())}",
        }, ensure_ascii=False)

    # ---- delete ----

    async def _delete(self, params, sf, FlowTestPlan, FlowTestPlanCase,
                      FlowTestPlanExecution, FlowTestPlanReview):
        plan_id = params.get("plan_id", "")
        ...
        if confirm != "确认删除":
            return json.dumps({
                "ok": False,
                "message": "删除测试计划需要用户确认。请让用户回复「确认删除」后再调用。",
            }, ensure_ascii=False)
    ...
    async def _add_cases(self, params, sf, member_id, FlowTestPlan, FlowTestPlanCase):
        ...
        async with sf() as session:
            ...
            existing = (await session.execute(
                select(FlowTestPlanCase.test_case_id).where(FlowTestPlanCase.plan_id == plan_id)
            )).scalars().all()
    ...
    async def _execute_case(self, params, sf, member_id, FlowTestPlanCase, FlowTestPlanExecution):
        ...
        async with sf() as session:
            ...
            ex = FlowTestPlanExecution(
                ...
                result=result_val,
                executor_id=member_id or None,
                notes=params.get("notes", ""),
                bug_id=params.get("bug_id") or None,
            )
            session.add(ex)
            await session.commit()
    ...
    async def _list(self, params, sf, Member, FlowTestPlan, FlowTestPlanCase,
        ...
        async with sf() as session:
            ...
            for plan in rows:
                ...
                total_cases = (await session.execute(
                    ...
                    .where(FlowTestPlanCase.plan_id == plan.id)
                )).scalar_one()
        ...
        return json.dumps({
            "ok": True, "count": len(items), "test_plans": items,
        }, ensure_ascii=False)
    ...
    async def _copy(self, params, sf, member_id, FlowTestPlan, FlowTestPlanCase):
        ...
        async with sf() as session:
            ...
            new_plan = FlowTestPlan(
                ...
                title=params.get("title") or f"{src.title} (副本)",
        ...
        return json.dumps({
            "ok": True,
            "message": f"测试计划已复制为「{new_plan.title}」，包含 {len(case_rows)} 条用例",
            "plan_id": new_plan.id,
        }, ensure_ascii=False)
    ...
    async def _add_review(self, params, sf, member_id, FlowTestPlan, FlowTestPlanReview):
        ...
        async with sf() as session:
            ...
            existing = (await session.execute(
                select(FlowTestPlanReview).where(
                    FlowTestPlanReview.plan_id == plan_id,
                    FlowTestPlanReview.repo_id == repo_id,
                    FlowTestPlanReview.pr_number == pr_number,
                )
            )).scalar_one_or_none()
            if existing:
        ...
        return json.dumps({
            "ok": True,
            "message": f"PR #{pr_number} 已添加到评审列表",
            "review_id": review.id,
        }, ensure_ascii=False)

    # ---- update_review ----

    async def _update_review(self, params, sf, member_id, Member,
        ...
        async with sf() as session:
            ...
            if reviewer_name:
                result = await session.execute(select(Member).where(Member.name == reviewer_name))
                m = result.scalar_one_or_none()
                if m:
                    old_reviewer = review.reviewer_id
                    review.reviewer_id = m.id
                    changes["reviewer"] = reviewer_name
            ...
            if params.get("review_status") and params["review_status"] != old_status:
                session.add(FlowTestPlanReviewHistory(
                    ...
                    old_status=old_status, new_status=params["review_status"],
                    notes=params.get("review_notes", ""), actor_id=member_id or None,
                ))
        ...
        return json.dumps({
            "ok": True,
            "message": f"评审项已更新，修改字段: {'、'.join(changes.keys())}",
        }, ensure_ascii=False)
    ...
    async def _list_reviews(self, params, sf, Member, FlowTestPlanReview):
        ...
        return json.dumps({
            "ok": True, "count": len(items), "reviews": items,
        }, ensure_ascii=False)
