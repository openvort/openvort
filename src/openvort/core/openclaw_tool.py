"""
OpenClaw remote work AI Tool.

Allows AI employees to delegate work to their bound
OpenClaw node on a remote machine.
"""
...
log = get_logger("core.openclaw_tool")

_service: OpenClawNodeService | None = None
_session_factory: async_sessionmaker[AsyncSession] | None = None


def set_openclaw_tool_runtime(
...
class OpenClawWorkTool(BaseTool):
    """Delegate work to a remote OpenClaw node bound to the current AI employee."""

    name = "openclaw_work"
    description = (
        "在 AI 员工绑定的远程电脑上执行工作任务。"
        "通过 OpenClaw 向远程机器发送指令，适用于编码、文件操作、命令执行等场景。"
        "需要 AI 员工已绑定远程工作节点。"
    )
    ...
    async def execute(self, params: dict) -> str:
        ...
        # Resolve the AI employee's bound node via _target_member_id
        target_member_id = params.get("_target_member_id", "")
        if not target_member_id:
            return (
                "当前不在 AI 员工聊天上下文中，无法使用远程工作节点。\n"
                "请先与一个已绑定 OpenClaw 节点的 AI 员工对话。"
            )

        # Look up member from DB to get openclaw_node_id
        ...
        async with _session_factory() as db:
            ...
            if member:
                node_id = member.openclaw_node_id or ""
        ...
        if not node_id:
            return (
                ...
                "请管理员在后台「系统管理 → OpenClaw 节点」中添加节点，"
                "并在「组织管理」中为该 AI 员工绑定节点。"
            )

        # Check node exists
        node = await _service.get_node(node_id)
        if not node:
            return f"绑定的 OpenClaw 节点（{node_id[:8]}...）不存在，请管理员重新配置。"

        if node.get("status") == "offline":
            return (
                f"远程节点「{node['name']}」当前离线（{node['gateway_url']}），"
                "请检查远程电脑是否开机且 OpenClaw 正在运行。"
            )
...
def get_openclaw_tools() -> list[BaseTool]:
    """Return all OpenClaw work tools."""
    return [OpenClawWorkTool()]
