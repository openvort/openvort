...
```bash
PYTHONUNBUFFERED=1 ~/.virtualenvs/openvort/bin/openvort start --web 2>&1
```

### 重启（推荐）

```bash
PYTHONUNBUFFERED=1 ~/.virtualenvs/openvort/bin/openvort restart --web 2>&1
```
...
```
Shell 1: PYTHONUNBUFFERED=1 ~/.virtualenvs/openvort/bin/openvort restart --web 2>&1
