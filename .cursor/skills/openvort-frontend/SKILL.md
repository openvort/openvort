...
<script setup>
import { Button, Input, Switch } from "@openvort/vort-ui";  // 禁止！
</script>
<template>
...
**唯一例外：** `Dialog`/`Drawer` 作为独立弹窗组件的根容器时，可从 `@openvort/vort-ui` 手动 import。
...
- **禁止**用底层 `VortDialogContent`/`VortDialogFooter` 拼装，必须用 `Dialog` from `@openvort/vort-ui`
...
### 规则 10：Drawer 宽度必须保证表单内容不被截断

配置类抽屉（含表单 + 指南文本）宽度不得小于 `580px`，确保 label + input + 描述文字完整显示。

| 抽屉内容 | 推荐 width |
|---------|-----------|
| 纯表单（4-6 字 label） | `520px` |
| 表单 + 配置指南/说明文字 | `580px` |
