...
**禁止使用 emoji 作为功能图标**（如 💻📋🧪🎨🤖），一律使用 lucide-vue-next 组件。
...
import { Code, ClipboardList, TestTube, Palette, Bot } from "lucide-vue-next";
</script>
<template>
    <Code :size="16" class="text-gray-500" />
    <!-- 动态图标用 component :is -->
    <component :is="iconMap[role]" :size="16" class="text-gray-500" />
</template>
```
