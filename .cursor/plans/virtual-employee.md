...
todos:
  ...
  - id: virtual_employee_module
    content: 实现AI 员工模块 virtual_employee/
  - id: skill_templates
...
├── attendance/           # 考勤模块（新建）- 真实+AI 员工共用
│   ├── models.py         # Attendance 打卡记录, Report 工作汇报
│   ├── service.py
│   ├── plugin.py         # AI Tools：打卡、提交汇报
│   └── router.py        # API
└── virtual_employee/    # AI 员工扩展（新建）
    ├── models.py         # AI 员工扩展配置
    ├── service.py
    ├── plugin.py         # AI Tools：AI 员工特有
...
```python
is_virtual: Mapped[bool] = mapped_column(Boolean, default=False)  # 是否AI 员工
...
1. **数据模型** - Member 扩展 + 考勤表 + 汇报表 + AI 员工配置表
2. **考勤模块** - service + plugin + router
3. **AI 员工模块** - service + plugin + router
4. **角色模板** - 创建 5 个预置模板
5. **前端考勤页面**
6. **前端AI 员工页面**
7. **通讯录集成**
